...
export function parseAttribute(node, attr, {props, attrs, events,  methods}) {
	let eventName = attr.name.match(/on(\w+)/);	
	if (eventName) {
		eventName = eventName[1];
		let eventHandler = methods[attr.value]
		if (eventHandler) {
			node.addEventListener(eventName, eventHandler.bind({props, attrs, events}), false);
		}
		return true;
...
export function parseNode(node, {dataPath, aliasKey, props, attrs, events, methods}) {
  if (!node) {
    return;
  }
  if (
  ...
  if (node.nodeType == 3) {
    ...
    node.nodeValue = parser(props);
		...
		for (let i = 0; i < node.attributes.length; i++) {
			let parsedAttr = parseAttribute(node, node.attributes[i], {props, attrs, events, methods});
	...
	if (xAttrs.forLoop === true) {
  	...
  	node.childNodes.forEach(node => parseNode(node, {aliasKey, dataPath, props, attrs,events, methods}));
	}
}
