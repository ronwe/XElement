...
var Parser = {
  loop: function(
		node, itemContent, xAttrs, method, attribute,
		property = {}, item, index, 
		{
			position = null, //插入位置
...
export function parseAttribute(node, attr, {property, attribute, method}) {
	let eventName = attr.name.match(/on(\w+)/);	
	if (eventName) {
		eventName = eventName[1];
		let eventHandler = method[attr.value]
		if (eventHandler) {
			node.addEventListener(eventName, eventHandler.bind({property, attribute}), false);
		}
		return true;
...
export function parseNode(node, {dataPath, aliasKey, property, attribute, method}) {
  if (!node) {
    return;
  }
  if (
  ...
  if (node.nodeType == 3) {
    ...
    node.nodeValue = parser(property);
		...
		for (let i = 0; i < node.attributes.length; i++) {
			let parsedAttr = parseAttribute(node, node.attributes[i], {property, attribute, method});
	...
	if (xAttrs.forLoop === true) {
      ...
      //console.log('xAttrs', xAttrs, dataPath, aliasKey);
		  let parser = Parser['loop'].bind(null, node, itemContent, xAttrs, method, attribute );;
      Dep.running = {
        node,
        type: 'loop',
  	...
  	node.childNodes.forEach(node => parseNode(node, {aliasKey, dataPath, property, attribute,method}));
	}
}
