...
function parseArgsString(str ) {
	...
	let checkQuote
	let tmp = ''
	function appendToResult(withQuoter) {
		if (tmp) {
			let piece = tmp;
			if (!withQuoter) {
				piece = piece.trim();
...
export function parseAttribute(node, attr, {props, attrs, events,  methods, bindedMethods}) {
	...
	if (eventName) {
		...
		//判断是否有参数
		let bindedMethdParsed = attr.value.match(/(\w+)\((.*)\)/);
		let methodName = attr.value;
		let methodArgs = [];
		if (bindedMethdParsed) {
			methodName = bindedMethdParsed[1];
			methodArgs = parseArgsString(bindedMethdParsed[2], props);
		}
		let eventHandler = bindedMethods[methodName];
		if (eventHandler) {
			node.addEventListener(eventName, function(evt) {
				let dymanicArgs = evt;
				if (methodArgs.length) {
					dymanicArgs = methodArgs.map(fakeArg => {
						let propName;
						if ('object' === typeof fakeArg && fakeArg[argsPropNameSymbol]) {
							propName = fakeArg[argsPropNameSymbol];
						}
						if (propName) {
				...
				eventHandler.apply(null, dymanicArgs);
			}, false);
			attr.value = '';
		}
		return true;
