...
/*
 * 监听props变化*/
export function addPropsWatch(props, watchStack, passArgs) {
	Object.keys(watchStack).forEach( toWatchProp => {
		if (toWatchProp in props) {
			Dep.running = {
				type: 'data',
				node: toWatchProp,
				compute: () => {
					watchStack[toWatchProp](passArgs);
				}
			};
			props[toWatchProp];
			Dep.running = null;
