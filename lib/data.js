 ...
 * 获取对象所有叶子节点对应key 
 */
function getUpdatedKeys(node, value) {
  mapNode(value, node);
}

function mapNode(value, root) {
  Object.keys(value).forEach(k => {
    if ('object' === typeof root[k]) {
      mapNode(value[k], value[k]);
    } else {
      let dataMapKey = getDataMapKey(root,k);			
      console.log(dataMapKey);
    }
  })
}
/*
...
function updateNode(target, value) {
  let parent = target[dataParentSymbol];
  let key = target[dataKeySymbol];
  target = value;
  if (parent) {
    target[dataParentSymbol] = parent;
  }
  if (key) {
    target[dataKeySymbol] = key;
  }
  return target;
}
	...
	function setProxy(data) {
        ...
        //console.log('set', target, key);
          ...
          if ('object' === typeof value && !Array.isArray(value)) {
            ...
            let updatedKeys = getUpdatedKeys(node, value);
          } else {
					  target[key] = value;
